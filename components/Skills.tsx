...
function Skills({skills}: Props) {
  const animation = [null, null, null, null, null, null]
  return (
    ...
    max-w-[2000px] xl:px-10 min-h-screen justify-center xl:space-y-0 mx-auto items-center'>
        ...
        <div className='absolute top-48 grid grid-cols-4 sm:grid-cols-5 gap-1 sm:gap-5'>
            {skills.map((skill, i) => (
              <Skill key={skill._id} skill={skill} ind={i} />
            ))}        
        </div>
    </motion.div>
  )
